      ...
      {flatShown ? (
        <p className="chart__degenerate">
          {flatExactly
            ...
            : `За горизонт значение не меняется на показанной точности: во всех ${points.length} точках ${shown[0]} ${unit}, а полный разброс между ними — ${spread.toExponential(1)} ${unit}, то есть он есть, но лежит за пределом отображаемых знаков. Плоская линия — это результат расчёта, а не ошибка отрисовки; шкала не растянута, чтобы этот разброс не выглядел значимым.`}
          {" "}
