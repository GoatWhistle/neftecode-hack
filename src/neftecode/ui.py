...
function render() {{
  ...
  if (d.status !== "refuse") {{
    ...
    body += card("Качество и запас до предела", `<table>${{rows(
      (e.statements || []).filter(s => ["sulfur_mgkg","t95_c","cetane_number"].includes(s.topic))
...
def render(payload: dict) -> str:
    ...
    return TEMPLATE.format(payload=json.dumps(payload, ensure_ascii=False, default=str))
