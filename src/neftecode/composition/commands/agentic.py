"""CLI handler for the deterministic agent demonstration (scripted policy, no language model, no network)."""
import json
...
def agent_demo(args, parser, root, out):
    ...
    for name in names:
        ...
        legacy = MakeDecision(scenario, robustness_evaluator=robustness).decide(budget=400, raw_scenario=document)
        agentic = AgenticMakeDecision(scenario, demo_llm(), settings=AgentSettings(), robustness_evaluator=robustness,
                                      response_effect=UnavailableResponseEffect()).decide(budget=400, raw_scenario=document)
