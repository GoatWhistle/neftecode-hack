...
Актуализация 2026-09-13 после переноса на Clean Architecture; 2026-09-14 — требования закрытой среды
из Q&A 11.09 и схемы АВТ с тегами; 2026-09-16 — слой LLM-агентов; 2026-09-17 — 307 как выброс, пороги доверия
из данных, официальные ВАК и справочник тегов 16.09, провайдер `local`. Концепция — `solution.md`, состояние —
`state.md`. Ниже архитектура с актуальными статусами компонентов; блоки целевых контрактов описывают смысл,
точные Python-поля — в коде.
...
Развертывание поддерживает два режима: локальный CLI собирает модульное приложение в одном процессе,
а сервисный режим запускает data, model, decision и gateway отдельными HTTP-процессами. Детерминированные
роли — программные, с типизированными входами/выходами внутри decision-контекста; поверх них слой LLM-агентов
(раздел 6), который может только сужать множество допустимых планов.
Брокер сообщений не нужен. Пользовательский интерфейс и CLI вызывают одно расчетное ядро.
Рекомендации адресованы оператору; подключение к автоматическому управлению оборудованием не входит
в текущий объем работ.

Среда эксплуатации по Q&A 11.09: технологическая сеть без интернета, локальный запуск обязателен и оценивается.
Детерминированное ядро, scripted-режим и локальные модели не требуют внешних API; зависимости фиксирует
`uv.lock`, модели обучаются и хранятся локально. Опциональный live-режим LLM-агентов по умолчанию ходит в Z.AI
(решение пользователя 17.09); для закрытой сети предусмотрен провайдер
`local` — OpenAI-совместимая модель завода (до ~30B), `LOCAL_LLM_MODEL`/`LOCAL_LLM_BASE_URL`, ключ не нужен. Без
доступной модели решение остаётся детерминированным с пометкой `fallback`.

Текущий стек ядра: Python 3.12, pandas, NumPy, scikit-learn, CatBoost, openpyxl, pytest, uv.
HTTP API реализован стандартным `http.server`, экран — React/TypeScript, собранный Vite. Серверных фреймворков нет;
...
| Tag registry | Семантика тегов, единицы, измеряемость/управляемость, статус подтверждения | Есть config/parameters.json и requirements-map.md; для АВТ — `config/avt_tags.json` с официальными описаниями 16.09, для 24-2000 — `control_tags_24_2000` по справочнику 16.09 (масштаб `F15` не согласован); карту читают загрузчик и отчёт ВАК |
...
| Data trust | Доверие к источникам, достаточность данных, выбор резервного режима | `application/services/trust.py`; пороги свежести/пропусков строятся из правил данных и конфигурации; 307 — пропуск; непригодное состояние блокирует запуск прогноза |
...
| Tank park | Партии основного компонента, стадии, паспорт, переменный отбор и массовый баланс | `ParkEvaluator` и `ParkState`: N=4 в текущих сценариях, 5000 м³, паспорт 12 ч, номинальный слив 24 ч; фактические стадии завода неизвестны |
| Quality agent | Проверка качества каждого кандидата | Сера/T95/цетан/плотность (двусторонний предел) в каждой точке траектории; missing/unknown блокирует. Это проверка Gate, а не отдельная статистическая оценка неопределённости |
...
| Look-ahead | Время до нарушения качества за горизонтом и упреждающий выбор | `PlanOperation.lookahead`, `MakeDecision._look_ahead`: сценарное продление до 48 ч и запас реакции; не ослабляет Gate |
| Control actuation | Как исполняется изменение: уставка регулятору с обратной связью, контур, тег | `ControlActuation` в сценарии; обязателен для каждой переменной, выводится в объяснение; динамика контура не моделируется |
| Optimizer | Генерация кандидатов, пересчет после запрета, допустимые альтернативы | Кодовый `CandidateGenerator`/`PlanOperation`: уставки АВТ/ГО, смесь, выпуск, присадка, постоянные и переходные планы; ограниченный бюджет. Отдельного LLM-агента оптимизации нет |
| Hard-constraint gate | Единая обязательная проверка кандидата и окончательного решения | `domain/advisory/gate.py`; полная сетка, park capacity/status/passport/balance, исправимость прогноза паспорта и terminal inventory при включённой политике |
| Coordinator | Порядок вызовов, выбор ветки/fallback, ограниченная петля поиска, отказ | `application/use_cases/make_decision.py` и `plan_operation.py`; veto агентов до ранжирования, feedback с новым содержимым, финальная перепроверка |
...
| Replay/evaluation | Воспроизведение, метрики, сценарии отказов, сравнение вариантов системы | `application/use_cases/ReplayDecisions` и `evaluation/`; раздельные history/simulated, pause/resume, пять сценариев (с `winter_grade`) |
| HTTP API / UI | Временная шкала, состояние цепочки, кандидаты, объяснение решения | React-интерфейс показывает серверную траекторию парка, партии и источники; стадии на клиенте не пересчитываются |
...
ParkTrajectory
  model_version, feasible, reasons[]
  frames[{time_h, inflow_t, outflow_t, state{tanks[]}}]
  tank: tank_id, batch_id, status, mass_t, capacity_t,
        properties, passport_ready_in_h, nominal_drain_tph, provenance
...
Сейчас явно реализованы сценарные модели цепочки, смешение и партийный парк. Их
параметры и отклик не являются подтверждённой цифровой моделью завода.
...
Текущее ранжирование: после проверок максимум выпуска, затем меньшая условная стоимость, тяжесть и число изменений; при явно заданном окне стоимости допускается выбор меньшей тяжести. Карта компромиссов показывает недоминируемый фронт исследованного допустимого пула; выбор ядра остаётся отдельной политикой.
...
LLM обязательных решений не принимает: слой агентов ниже не может изменять числа, ограничения или результаты
проверок — только сужать выбор.

### Слой LLM-агентов (2026-09-16; включён по умолчанию, явное отключение — `AGENTIC_DECISION_ENABLED=0`)

Поверх описанного контура добавлен ограниченный multi-agent слой. Роли: `OrchestratorAgent` (координация),
`QualityAgent` (проверка качества и Gate), `ReliabilityAgent` (ограничения, границы,
устойчивость). Они вызывают только детерминированные инструменты (`application/agentic/tools.py`), каждый
со своим allowlist и схемой аргументов; результат инструмента — компактный JSON с ссылкой evidence_ref.
Порядок полномочий: legacy `MakeDecision` считается первым и остаётся эталоном и запасным вариантом; агенты
могут только сузить множество допустимых планов (закрытый словарь ужесточающих ограничений и вето по
кандидату) или потребовать отказа при обоснованном REJECT; выбор среди допустимых делает `rank()`; план
проходит `release` (упреждение, финальная перепроверка Gate, устойчивость) и повторную переоценку свежим
`PlanOperation`. После сбоя уже принятые ограничения и veto сохраняются; при пустом допустимом множестве
выдаётся отказ. Отдельный `evaluation/independent.py` проверяет стратегии в зафиксированных сценарных средах с отличающимися
откликом, задержкой и свойствами смешения, недоступными оптимизатору. Это не независимый физический симулятор:
используется то же ядро `PlanOperation` с изменёнными параметрами, а опубликованный артефакт посчитан при
ограниченном бюджете поиска 400 (не при runtime-default 1200); результат нельзя переносить на завод.

Размещение: `application/ports/llm.py` (порт), `application/agentic/*` (контракты, бюджет, сессия,
инструменты, цикл, агенты, решение), `infrastructure/llm/*` (Z.AI Coding Plan, OpenAI-совместимый,
Anthropic, `local` — OpenAI-совместимая модель закрытой сети, детерминированные заглушки),
`infrastructure/agentic/factory.py` (флаг и сборка).
Сеть — только stdlib `urllib`; новых зависимостей нет. С 16.09 слой включён по умолчанию (решение пользователя);
`AGENTIC_DECISION_ENABLED=0` отключает его явно — тогда конфигурация не читается и решения байт-в-байт
прежние (так работает набор тестов). Требование закрытой среды: OpenAI-совместимый адаптер направляется на
локальную LLM завода (`LLM_PROVIDER=local`), либо агенты отключаются явно. Бюджет: не больше 12 вызовов модели,
shortlist из 5 кандидатов. Решение с живой моделью занимает минуты; gateway ждёт decision до 660 с. Контракты и ограничения описаны в `application/agentic/` и тестах.
...
research/                     # методики и результаты исследований
...
`bootstrap.py` запускает CLI через `composition/commands/dispatcher.py`. Пакет `composition/`
...
Старые плоские модули удалены; CLI-команды и JSON решений сохранены. Проверки запускаются командой `LLM_PROVIDER=scripted uv run pytest -q`.
...
расширенные поля (например property_uncertainty) представлены буквально. Локальный режим и единое
ядро решения сохраняются; дополнительно доступен запуск четырёх процессов.
...
Gate проверяет серу, T95, цетановое число, плотность (нижний и верхний предел), массовые доли, доступные остатки и прочие явно заданные пределы **на каждом шаге**. Неизвестное критическое условие блокирует план. Финальное действие проверяется повторно; низкий риск-скор не отменяет проверку. Конечный запас проверяется дополнительно только если сценарий включил `terminal_inventory_rule`; по умолчанию такого правила нет.
...
Ключевые параметры (сверены при реализации): горизонт/отклик 0–3 часа, ЛИМС до 4 часов после отбора, управляющие ГО по справочнику 16.09 (`T6` — вход реактора, `F15`/`F9` — сырьё; масштаб `F15` не согласован), официальные формулы ВАК 16.09, четыре свойства смеси (сера, T95, цетан, плотность; пределы ЦЧ и плотности — от экспертов 582) и условия присадки. Развёртывание — закрытая среда, регуляторы с обратной связью.
# Process deployment

На уровне deployment монорепозитория есть четыре независимых процесса: data, model, decision и
gateway. Они обмениваются только JSON HTTP-контрактами через `services.common`; реализации друг
друга не импортируют. Supervisor `neftecode-stack` запускает их отдельными Python subprocess,
проверяет health/readiness и завершает группу при падении. Data readiness требует валидных
сценариев, а доступность `task/` показывается отдельной capability; model readiness требует
валидные `model.pkl` и `manifest.json`. Evaluation остаётся batch CLI.
