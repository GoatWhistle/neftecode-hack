...
def test_drawing_from_the_execution_state_does_not_mutate_it():
    state = ExecutionState.from_scenario(load_scenario(SOUR))
    before = state.tanks["main"].inventory_t
    state.draw({"main": 10.0})
    assert state.tanks["main"].inventory_t == before


def test_a_draw_returns_a_new_state_with_the_stock_reduced():
    state = ExecutionState.from_scenario(load_scenario(SOUR))
    after = state.draw({"main": 10.0})
    assert after.tanks["main"].inventory_t == pytest.approx(
        state.tanks["main"].inventory_t - 10.0)


def test_a_draw_of_an_on_demand_component_is_recorded_as_production():
    state = ExecutionState.from_scenario(load_scenario(SOUR))
    after = state.draw({"reserve": 10.0})
    assert after.tanks["reserve"].produced_t == pytest.approx(
        state.tanks["reserve"].produced_t + 10.0)
    assert after.tanks["reserve"].inventory_t == pytest.approx(0.0)


def test_simulated_run_spends_current_recipe_between_moments():
    state = ExecutionState.from_scenario(load_scenario(SOUR))
    initial = state.tanks["reserve"].produced_t
    result = replay().run([{"at": "2026-01-05T08:00:00"},
                           {"at": "2026-01-05T09:00:00"}], SIMULATED)
    assert result["final_execution"]["tanks"]["reserve"]["produced_t"] > initial


def test_one_tonne_per_hour_of_reserve_changes_decision_and_never_proposes_twenty_percent():
    r = replay()
    state = ExecutionState.from_scenario(load_scenario(SOUR))
    low = replace(state, tanks={**state.tanks,
                                "reserve": replace(state.tanks["reserve"], max_outflow_tph=1.0)})
