...
from neftecode.application.services.trust import (DataTrustAgent, MISSING, OK, PLACEHOLDER_VALUE, SOURCE_PRIORITY,
                             SourceVerdict, UNUSABLE, inspect_value)
...
def test_flagged_raw_values_reach_the_report_without_blocking_it():
    ...
    assert report.usable is True, "отметка отдельного значения сама по себе не отменяет решение"


def test_only_an_ok_source_may_carry_a_decision():
    for status in (UNUSABLE, MISSING, "suspect", "", "OK"):
        verdict = SourceVerdict("ЛИМС", status)
        assert verdict.usable is False, status
    assert SourceVerdict("ЛИМС", OK).usable is True


def test_the_trust_model_has_exactly_two_source_outcomes():
    module = importlib.import_module("neftecode.application.services.trust")
    statuses = {value for name, value in vars(module).items()
                if name.isupper() and isinstance(value, str) and name in {"OK", "UNUSABLE", "MISSING"}}
    assert statuses == {"ok", "unusable", "missing"}
    assert not hasattr(module, "SUSPECT"), "промежуточный статус не реализован: объявлять его нельзя"
    assert not hasattr(SourceVerdict, "reportable"), "мёртвое свойство вернулось"


def test_an_unusable_source_never_becomes_the_primary_one():
    report = assess(lab_usable=False, pak_frozen=True)
    assert report.verdict("ЛИМС").status == UNUSABLE
    assert report.verdict("ПАК").status == UNUSABLE
    assert report.primary is None and report.usable is False
