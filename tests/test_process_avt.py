...
# --- The model is built from the scenario, not from hidden constants ---

def test_model_coefficients_come_from_the_scenario():
    model = stage().model
    raw = json.loads(BASELINE.read_text())["stages"]["avt"]["model"]
...
# --- Declared arithmetic, checked by hand ---
...
# --- Directions of the declared dependencies ---
...
def test_worse_crude_raises_diesel_sulfur_without_touching_the_flow():
    base = stage().run()
    sour = AvtStage(parse_scenario(json.loads(Path("config/scenarios/sour_crude.json").read_text()))).run()
...
# --- Outside the declared region the result says so ---
...
def test_the_stage_needs_no_history_only_the_scenario_and_the_setpoints():
    """A response model answers 'what if', so it must not require past observations."""
