...
def history(days=60, seed=0):
    ...
    pak.loc[1000:1011, "value"] = 12.3456
    ...
    signals.iloc[::50, :1] = np.nan
    signals.iloc[7::97, :12] = np.nan
...
def test_rules_are_derived_from_the_history_with_the_method_recorded():
    ...
    assert rules["pak_frozen_readings"] == 2
    assert 1.0 < rules["pak_conflict_mgkg"] < 3.0
    assert rules["telemetry_max_missing_fraction"] < 12 / 20
    assert rules["telemetry_max_missing_fraction"] >= 1 / 20
...
def test_frozen_and_conflict_rules_follow_the_configuration():
    ...
    assert conflict_mask([10.0], [5.0], {}).tolist() == [True]
