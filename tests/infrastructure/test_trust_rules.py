...
ROOT = Path(".")
EXPERIMENT = json.loads((ROOT / "config/experiment.json").read_text(encoding="utf-8"))
...
def test_broken_artifact_is_an_error_not_a_silent_fallback(tmp_path):
    ...
    (root / "artifacts" / "source_rules.json").write_text(json.dumps({"schema_version": "v0", "rules": {}}), encoding="utf-8")
    with pytest.raises(ValueError, match="schema_version"):
        load_trust_rules(root, root / "artifacts")
    (root / "artifacts" / "source_rules.json").write_text(
        json.dumps({"schema_version": "v1", "rules": {"lab_max_age_hours": 48}}), encoding="utf-8")
...
def test_local_snapshot_provider_uses_the_model_thresholds(monkeypatch):
    ...
    monkeypatch.setattr("neftecode.infrastructure.live.providers.state_at", lambda *args: stale_lab_state())
