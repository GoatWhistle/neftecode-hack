...
def test_scenario_quantities_are_never_reported_as_measured():
    ...
    for tank in scenario.tanks:
        if tank.on_demand:
            assert tank.inventory.source == "derived" and tank.cost_per_t.source == "derived"
            continue
...
def test_describe_reports_gaps_rather_than_hiding_them():
    ...
    assert summary["available_tanks"] == ["main", "reserve"], "лёгкий компонент выключен в baseline"
    assert summary["total_inventory_t"] == 4000.0, "запас считается только по складам, резерв производится"
