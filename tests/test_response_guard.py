...
#: Задержка как в сценарии (2 ч, весь ход): ход T6 действует внутри горизонта 3 ч, и Gate на горизонте решает.
...
def knife_edge(light: bool = False, tank_sulfur: float = 9.34, upper: float = 10.6, **response_overrides) -> dict:
    """Резервуар 600 т с серой 9.34, приток 10.6, задержка 2 ч: без хода T6 предел нарушается на 3 ч, при β ход
    +1 °C спасает (9.98), при слабом крае −0.217 — нет (10.016). Резерв недоступен, ходы АВТ отключены."""
...
def test_without_a_data_driven_slope_there_is_no_weak_edge():
    assert weak_response_raw(json.loads(BASELINE.read_text())) is None
...
def test_a_move_justified_only_past_the_horizon_must_keep_its_promise_at_the_weak_edge():
    """План выбирает look-ahead: при β +1 °C убирает нарушение из 48 ч, при слабом крае оно наступает
    раньше запаса реакции 12 ч (при задержке исследования 3 ч — через 8.5 ч)."""
