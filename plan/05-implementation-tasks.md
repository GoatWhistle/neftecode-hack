...
### [x] T73. Один live smoke GLM-5.3-Flash
...
- Готово: перед запуском напечатаны provider, model, base_url, max_calls, scenario id (без ключа); ровно один запуск
  `--live` 2026-09-16; провайдер вернул ошибку класса `quota` на первом запросе, слой откатился к legacy (hold, Gate PASS).
  Повторный live — только по новому поручению пользователя. В trace добавлены код и текст ошибки провайдера.
- Коммит: `docs: зафиксировать живую проверку агентного режима`.

## Фаза 10 — аудит

### [x] T74. Аудит, документация, итог
