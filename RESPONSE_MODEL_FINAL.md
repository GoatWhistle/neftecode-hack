# Модель отклика серы на температуру ГО — финальная спецификация (T11-версия, история)

> **Статус на 2026-09-17.** Основа этой версии — `T11` как вход реактора и `F26` как расход сырья — **опровергнута**
> официальным справочником тегов 24-2000 от 16.09 (`context/materials-2026-09-16.md`, `config/parameters.json`):
> `T11` — выход Р-202, `F26` — гидроочищенный ДТ в цех №8. **Актуальная версия — `RESPONSE_MODEL_T6.md`** (вход `ht.T6`,
> расход `ht.F9`; production-оценка `train` на τ = 2026-01-01: β = −0.423 [−0.477; −0.389], `artifacts/response_model.json`;
> политика — `config/response_model.json`).
> Текст ниже сохранён без изменений как история и как описание метода (метод в T6-версии тот же).
