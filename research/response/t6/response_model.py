"""T6/F9 re-estimation of the accepted temperature-response model (copy of final/response_model.py).

Only the tags changed: reactor-inlet temperature = ht.T6 (official dictionary 16.09), feed = ht.F9 (mass, t/h).
Running rule: F9 above a quantile floor (q01 of F9 on thermally-running rows) instead of F26 > 150 and the F19/F26 band.
Column names T6/F9 replace the old T11/F26 throughout; `prepare` accepts other tags for like-for-like comparison.
...
Units: PAK in mg/kg (analyser ppm), T6 in °C, ΔT = sustained change of the reactor-inlet temperature setpoint, °C.
"""
...
SUPPORT_COLS = ("T6", "T6_30d", "F9", "h2oil")
...
# ------------------------------------------------------------------ preprocessing
def prepare(signals: pd.DataFrame, online: pd.DataFrame, frozen_rule: str = "past",
            t_tag: str = "ht.T6", f_tag: str = "ht.F9", feed_floor_q: float = 0.01) -> pd.DataFrame:
    """10-min frame. All flags use only the past except `stable_label` (training row selection only).

    Running = T6, T5 > 320 °C, P13 > 3 MPa, F2 > 30000, feed > 0 and feed > q(feed_floor_q) of feed on thermally-running
    rows, no missing value among the core tags (307 stubs are already NaN after load_sources).
    """
    idx = signals.index
    f = pd.DataFrame(index=idx)
    f["T6"], f["T5"], f["F9"] = signals[t_tag], signals["ht.T5"], signals[f_tag]
    f["P13"], f["F2"] = signals["ht.P13"], signals["ht.F2"]
    f["h2oil"] = f.F2 / f.F9
    hot = (f.T6 > 320) & (f.T5 > 320) & (f.P13 > 3.0) & (f.F2 > 30000) & (f.F9 > 0)
    feed_floor = float(f.F9[hot].quantile(feed_floor_q))
    core = [t_tag, "ht.T5", f_tag, "ht.P13", "ht.F2", "ht.T6", "ht.T11"]
    running = hot & (f.F9 > feed_floor) & signals[core].notna().all(axis=1)
    f["running"] = running
    ...
    f["T6_30d"] = f.T6.where(f.run_past12h).rolling("30d", min_periods=1000).mean()
    return f
...
def rows(f: pd.DataFrame, times: pd.DatetimeIndex) -> pd.DataFrame:
    ...
    d["T6_30d"] = f.T6_30d.reindex(times).to_numpy()
    ...
    pre_T = d.T6 - _wmean(f.T6, times, -90, -60, 1)
...
def arx_plateau(f: pd.DataFrame, start, end, boot: int = 0, seed: int = 0):
    """β: mean cumulative PAK response at 3–8 h to a permanent +1 °C T6 step; ARX on 10-min differences in (start, end]."""
...
def fit(f: pd.DataFrame, R: pd.DataFrame, tau, boot: int = 40) -> Artifact:
    ...
    return Artifact(
        ...
        T6_q01=float(H.T6.quantile(.01)), T6_q99=float(H.T6.quantile(.99)),
        F9_q01=float(H.F9.quantile(.01)), F9_q99=float(H.F9.quantile(.99)),
...
def live_state(f: pd.DataFrame, t) -> dict:
    ...
    keys = ["PAK30m", "PAK24h", "PAK24h_cov", "T6", "T6_30d", "F9", "h2oil", "run_past12h", "pak_frozen_now"]
...
def predict(art: Artifact, state: dict, dT: float) -> dict:
    ...
    if dT != 0:
        ...
        if not (art.T6_q01 <= state["T6"] <= art.T6_q99) or not (art.T6_q01 <= state["T6"] + dT <= art.T6_q99):
            reasons.append("T6 or T6+ΔT outside history q01–q99")
        if not (art.F9_q01 <= state["F9"] <= art.F9_q99):
            reasons.append("F9 outside history q01–q99")
