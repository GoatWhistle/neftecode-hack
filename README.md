...
<!-- СКРИНШОТ 1: общий вид интерфейса после расчёта.
     Что показать: сцена «риск качества» доведена до конца — видны все восемь этапов
     пайплайна на схеме, слева рейка прогресса, справа карточка ответа оператору
     со статусом recommend_scenario. Нужен полный экран в светлой теме.
     Файл: docs/screenshots/01-overview.png -->
...
<!-- СКРИНШОТ 5: карточка агентного слоя.
     Что показать: провайдер, модель, исход, число вызовов инструментов, мнения
     специалистов и применённые ужесточения. Если есть fallback — с явной причиной.
     Доказывает мультиагентность делом, а не описанием.
     Файл: docs/screenshots/05-agents.png -->
