...
Код: `src/neftecode/` — 203 модуля, разделённые на `domain`, `application`,
`infrastructure`, `evaluation`, `presentation`, `services`, `composition`.
Направления зависимостей проверяются AST-тестами в `tests/architecture/`.
Интерфейс: `src/frontend/` ([разработка](src/frontend/README.md)), собранная версия
входит в Git — Node.js для запуска не нужен.
