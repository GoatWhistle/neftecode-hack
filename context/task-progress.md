...
заблокировано 0. Этапы 0–2 закрыты по серверу, всё фронтовое описано в `frontend-handoff.md`. Текущий этап: 3 (A1–A3 закрыты). С A2 после слияния гоняется быстрый набор, полный `pytest` — в конце этапа (решение владельца 21.09).
