...
- `bootstrap.py` связывает слои и содержит обработчики старых CLI-команд.
...
`bootstrap.py` пока слишком большой: он одновременно собирает зависимости и содержит команды
обучения, demo, benchmark и генерации артефактов. Код работает, но для чтения его лучше делить
мысленно по функциям. Позже обработчики команд можно разнести по отдельным модулям.
