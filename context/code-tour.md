...
- `ml/runtime.py` — сбор исторического решения на выбранный момент.
- `ml/support.py` — историческая опора управляющего совета.
- `ml/twins.py` — похожие исторические случаи.
- `ml/attribution.py` — вклад участков цепочки.
- `ml/claims.py` — журнал утверждений и источников.
...
В проекте есть два похожих контура агентов:

1. `application/use_cases/make_decision.py` — основной новый DDD-контур сценарного решения.
2. `infrastructure/ml/agents.py::Coordinator` — более ранний словарный контур, который ещё нужен
   для старых синтетических demo-артефактов и исторического runtime.

Для понимания актуальной логики сначала читать первый. Второй — после того, как основной путь уже
понятен. Это реальный технический долг: два контура стоит в будущем объединить.

`bootstrap.py` также слишком большой: он одновременно собирает зависимости и содержит команды
