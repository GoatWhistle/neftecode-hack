...
| B5 | ✅ выполнено | `093a97d` | Целевые cancellation/SSE/safety-тесты — 14 passed, 39 deselected | Закрытие потока отменяет дальнейший поиск и новые LLM-вызовы; тестовый worker освобождается менее чем за 0,5 с. Уже отправленный внешний запрос нельзя остановить на стороне провайдера: после возврата ответ отбрасывается и новые вызовы не начинаются. |
| B6 | ✅ выполнено | текущий коммит | Архитектура — 22 passed; полный `LLM_PROVIDER=scripted uv run pytest -q` — 1410 passed за 288,32 с; stack SSE — `phase → phase → screen → end` | Реальные smoke: 05.01 hold/last_pak_bc; 24.07 recommend/last_pak_bc; 24.07 frozen_pak hold/catboost_no_pak; both_broken и no_feasible refuse; agents-off hold + disabled/skipped; unavailable и legacy snapshot дают явный отказ. Frontend не менялся. |
