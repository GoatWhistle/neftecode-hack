...
| Устойчивость к ошибкам отклика/задержки | `src/neftecode/evaluation/robustness.py`, `src/neftecode/application/use_cases/make_decision.py` | `tests/evaluation/test_robustness.py`, `tests/agentic/test_safety.py` | Возмущения сценарные; same-model проверка; обязательный стресс неприменим без хода ГО |
