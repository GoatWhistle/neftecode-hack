...
проходит `release` (упреждение, финальная перепроверка Gate, устойчивость) и независимую переоценку свежим
...
Перед реализацией сверить параметры с `organizer-clarifications.md`: горизонт/отклик 0–3 часа, ЛИМС до 4 часов после отбора, управляющие ГО по справочнику 16.09 (`T6` — вход реактора, `F15`/`F9` — сырьё; масштаб `F15` не согласован), официальные формулы ВАК 16.09, четыре свойства смеси (сера, T95, цетан, плотность; пределы ЦЧ и плотности — от экспертов 582) и условия присадки. Для АВТ — карту приборов `avt-schemes-2026-09-14.md`; для развёртывания — закрытую среду и регуляторы с обратной связью из `qa-session-2026-09-11.md`.
# Process deployment
