...
Текущее ранжирование: после проверок максимум выпуска, затем меньшая условная стоимость, тяжесть и число изменений; при явно заданном окне стоимости допускается выбор меньшей тяжести. Парето-фронт не строится.
...
Старые плоские модули удалены; CLI-команды и JSON решений сохранены. Актуальное число тестов и дату последнего полного прогона см. в `context/state.md`; оно меняется вместе с репозиторием.
