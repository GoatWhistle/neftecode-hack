...
Проект не обращается к внешним API; зависимости фиксирует `uv.lock`, модели обучаются и хранятся локально.
Слой LLM-агентов по умолчанию ходит в Z.AI (решение пользователя 17.09); для закрытой сети предусмотрен провайдер
...
| Data agent | Доверие к источникам, достаточность данных, выбор резервного режима | `application/services/trust.py`; пороги выведены из истории при обучении и хранятся в `model.pkl` (T76); 307 — пропуск (T75); непригодное состояние блокирует запуск прогноза |
...
| Quality agent | Проверка качества каждого кандидата с учетом неопределенности | S/T95/цетан/плотность (двусторонний предел) в каждой точке траектории, missing/unknown блокирует |
...
| Look-ahead | Время до нарушения качества за горизонтом и упреждающий выбор | `PlanOperation.lookahead`, `MakeDecision._look_ahead` (T54): 48 ч, запас реакции 12 ч, не ослабляет gate |
| Control actuation | Как исполняется изменение: уставка регулятору с обратной связью, контур, тег | `ControlActuation` в сценарии (T47); обязателен для каждой переменной, выводится в объяснение; динамика контура не моделируется |
...
### Слой LLM-агентов (2026-09-16; включён всегда, явное отключение — `AGENTIC_DECISION_ENABLED=0`)

Поверх описанного контура добавлен ограниченный multi-agent слой. Роли: `OrchestratorAgent` (координация),
`QualityAgent` (запас качества, неопределённость, данные), `ReliabilityAgent` (вмешательства, границы,
...
выдаётся отказ. Отдельный `evaluation/independent.py` проверяет стратегии в фиксированных средах с отличающимися
откликом, задержкой и свойствами смешения, недоступными оптимизатору.
...
Сеть — только stdlib `urllib`; новых зависимостей нет. С 16.09 слой включён всегда (решение пользователя);
...
shortlist из 5 кандидатов. Решение с живой моделью занимает минуты; gateway ждёт decision до 660 с. Полное описание, контракты и риски — `plan/00`–`plan/08`.
...
Старые плоские модули удалены; CLI-команды и JSON решений сохранены. Последняя проверка (2026-09-17): 1115 passed (агенты в тестах отключены через conftest); чистый комплект с этим набором не пересобирался.
