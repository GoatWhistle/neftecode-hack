...
из Q&A 11.09 и схемы АВТ с тегами. Концепция — `solution.md`, состояние —
...
а сервисный режим запускает data, model, decision и gateway отдельными HTTP-процессами. Агенты
остаются программными ролями с типизированными входами/выходами внутри decision-контекста.
...
Локальная LLM завода (OpenAI-совместимое API, до ~30B) допустима, но не используется; если появится, то только
для текста по готовому Decision.
...
| Tag registry | Семантика тегов, единицы, измеряемость/управляемость, статус подтверждения | Есть config/parameters.json и requirements-map.md; для АВТ добавлены приборы со схем 14.09 (`avt-schemes-2026-09-14.md`), масштабы части управляющих тегов не подтверждены; код карту не читает |
...
Anthropic, детерминированные заглушки), `infrastructure/agentic/factory.py` (флаг и сборка).
...
локальную LLM завода, либо агенты отключаются явно. Решение с живой моделью занимает минуты; gateway ждёт
decision до 660 с. Полное описание, контракты и риски — `plan/00`–`plan/08`.
...
Старые плоские модули удалены; CLI-команды и JSON решений сохранены. Последняя проверка (2026-09-15): 821 passed без предупреждений; чистый комплект — 820 passed, 1 skip.
...
Перед реализацией сверить параметры с `organizer-clarifications.md`: горизонт/отклик 0–3 часа, ЛИМС до 4 часов после отбора, управляющие ГО P8/T11/F19 с неразрешёнными вопросами масштаба, исправления ВАК, три свойства смеси и условия присадки. Для АВТ — карту приборов `avt-schemes-2026-09-14.md`; для развёртывания — закрытую среду и регуляторы с обратной связью из `qa-session-2026-09-11.md`.
# Process deployment
