...
Актуализация 2026-09-12 после реализации и аудита. Концепция — `solution.md`, состояние —
...
| Data agent | Доверие к источникам, достаточность данных, выбор резервного режима | Реализовано в trust.py; непригодное состояние блокирует запуск прогноза |
...
| Hard-constraint gate | Единая обязательная проверка кандидата и окончательного решения | gate.py; проверка полной сетки, обязательных полей и конечного остатка |
| Coordinator | Порядок вызовов, выбор ветки/fallback, ограниченная петля поиска, отказ | orchestrator.py; veto агентов до ранжирования, feedback с новым содержимым, финальная перепроверка |
...
| Replay/evaluation | Воспроизведение, метрики, сценарии отказов, сравнение вариантов системы | Раздельные history/simulated; фактические запасы/свойства/часы/операция, pause/resume; сравнение четырёх сценариев |
| HTTP API / UI | Временная шкала, состояние цепочки, кандидаты, объяснение решения | server.py/ui.py; одношаговый рецепт и все изменения видны |
...
## 9. Состояние архитектуры после реализации

Общие контракты, gate, модельная цепочка, динамика запасов, агенты, replay и UI/API реализованы.
Тестирование: 739 passed. Обязательная неизвестность блокирует план; текущий рецепт, доза и
...
Этот цикл реализован в planner/inventory/orchestrator/replay. Ниже сохранены смысловые требования контрактов; не все расширенные поля (например property_uncertainty) представлены буквально. Python-монолит и единое ядро CLI/UI сохраняются.
