...
Актуализация 2026-09-12: основная концепция и приоритеты — `solution.md`, фактический статус — `state.md`, поздние ответы экспертов — `organizer-clarifications.md`. Ниже сохранена архитектура 9 сентября; расширение для плана во времени приведено в разделе 10. Упоминания «сейчас» в исходных разделах описывают базовый прототип на дату их написания, а не готовность поздних экспериментальных модулей. Сейчас разрешено обновление контекста, разработка приостановлена.

Дата: 2026-09-09. Статус: целевая архитектура предложенного решения, с привязкой к текущему прототипу.
Запрос: технически описать структуру идеи и сохранить контекст. В этом шаге код не реорганизуем.
...
HTTP-фреймворк и frontend-фреймворк пока не зафиксированы: в репозитории их нет. Выбор не должен
влиять на контракты расчетного ядра.
...
| Tag registry | Семантика тегов, единицы, измеряемость/управляемость, статус подтверждения | Требуется; противоречия описаны в task-review |
| State builder | Временной срез по доступности, лаги/окна, возраст данных | Реализовано в data.py; задержка ЛИМС пока параметр эксперимента |
| Data agent | Доверие к источникам, достаточность данных, выбор резервного режима | Частично: свежесть/замерзание/расхождение ПАК и доля пропусков |
| Forecast models | Прогноз будущего качества с диапазоном | Только сера после гидроочистки; качество пока недостаточное |
...
| Action-effect model | Оценка изменения цепочки после конкретного действия | Только модельное смешение; АВТ/ГО не реализованы |
| Quality agent | Проверка качества каждого кандидата с учетом неопределенности | Сера смеси по верхним оценкам; другие свойства не покрыты |
| Reliability agent | Ограничения оборудования, объяснимые показатели нагрузки; ограничения переключений только как явная политика сценария | Только сценарный насос/предел изменения доли; не промышленная надежность. По позднему ответу эксперта ограничения скорости в кейсе не заданы |
| Optimizer agent | Генерация кандидатов, пересчет после запрета, допустимые альтернативы | Дискретный поиск по доле резерва и расходу; без управления АВТ/ГО |
| Hard-constraint gate | Единая обязательная проверка кандидата и окончательного решения | Проверки встроены в agents.py; отдельного общего модуля пока нет |
| Coordinator | Порядок вызовов, выбор ветки/fallback, ограниченная петля поиска, отказ | Реализован для текущего сценария; ранний выход при первом допустимом проходе |
...
| Replay/evaluation | Воспроизведение, метрики, сценарии отказов, сравнение вариантов системы | Частично: один временной тест, синтетические сбои, журналы примеров |
| HTTP API / UI | Временная шкала, состояние цепочки, кандидаты, объяснение решения | Не реализованы; есть CLI |
...
Это ориентир разделения ответственности, а не команда немедленно создать все каталоги.
Существующие файлы разделяем по мере добавления соответствующих возможностей.

```text
src/neftecode/
  contracts.py                # общие типы и их валидация
  data/                       # adapters, registry, time alignment, state builder
  models/                     # features, forecast, risk, calibration, training
  process/                    # action effects: avt, hydrotreating, blending
  agents/                     # data, quality, reliability, optimizer
  optimization/               # candidates, constraints, ranking
  engine/                     # coordinator, runtime, decision log
  evaluation/                 # replay, temporal splits, metrics, fault scenarios
  interfaces/                 # CLI и HTTP-адаптер
ui/                           # интерфейс, без собственной расчетной логики
...
Сейчас фактические файлы плоские: `data.py`, `forecast.py`, `risk.py`, `agents.py`,
`runtime.py`, `cli.py`. Реорганизация сама по себе не дает преимущества на хакатоне.
...
## 9. Следующие архитектурно значимые шаги

1. Оформить общие контракты, реестр тегов/ограничений и единый Gate без изменения результатов.
2. Подтвердить 3–5 управляющих параметров и собрать ограниченную проверяемую модель их последствий
   для АВТ/ГО. До этого сохранять модельный scope и явные ограничения применимости.
3. Улучшать прогноз и риск на временных окнах, учитывая режимы и запаздывания. Не увеличивать
   сложность модели без измеримого преимущества.
4. Добавить UI/API к существующему runtime: состояние цепочки, временная шкала, прогноз,
   кандидаты, ограничения и журнал взаимодействия. Расчеты и правила остаются в ядре.

Источники внутри проекта: `task-review/README.md`, `task-review/spec-text.txt`, `build-plan.md`,
текущие `src/neftecode/`, `config/`, `README.md`. Для архитектуры новых внешних фактов не использовано.

## 10. Расширение концепции от 12 сентября: план во времени

Это дополнение целевой архитектуры, не описание реализованных классов. Python-монолит, отдельные offline/runtime-контуры и единое ядро CLI/UI сохраняются. Текущая реализация пока не поддерживает полный перечисленный цикл.
