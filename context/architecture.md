...
### Слой LLM-агентов (2026-09-16, флаг `AGENTIC_DECISION_ENABLED`, по умолчанию выключен)
...
Сеть — только stdlib `urllib`; новых зависимостей нет; при выключенном флаге конфигурация не читается и
решения байт-в-байт прежние. Требование закрытой среды сохраняется: OpenAI-совместимый адаптер можно
направить на локальную LLM завода. Полное описание, контракты и риски — `plan/00`–`plan/08`.
